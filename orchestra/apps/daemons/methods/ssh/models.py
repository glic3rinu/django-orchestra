...
class SSHOption(models.Model):
    ...
    user = models.CharField("SSH User", max_length=32, default=settings.DAEMONS_DEFAULT_SSH_USER, blank=True)
    password = models.CharField("SSH Password", max_length=64, blank=True)
    host_keys = models.CharField("SSH Host Keys", max_length=255, blank=True, default=settings.DAEMONS_DEFAULT_SSH_HOST_KEYS)
    private_key = models.CharField("SSH Private Key", max_length=255, blank=True)
    port = models.IntegerField("SSH Port", default=settings.DAEMONS_DEFAULT_SSH_PORT, blank=True)
